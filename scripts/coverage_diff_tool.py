...
ALL_APPS = ['ActivityDiary', 'chess','AmazeFileManager', 'and-bible', 'AnkiDroid', 'APhotoManager', 'commons',
            ...
            'sunflower', 'WordPress', "AntennaPod", "newpipe", 'duckduckgo', 'RedReader', 'owntracks', 'myExpenses', 'bookdash', 'Wikipedia',
              'kotatsu', 'aard2','alarm','vanilla','app-gms-debug','feeder']


def get_app_name(testing_result_dir):
    for app_name in ALL_APPS:
        if os.path.basename(testing_result_dir).startswith(app_name):
            print(f"[INFO] Found app_name: {app_name} for testing result dir: {testing_result_dir}")
            return app_name
...
def get_class_source_files_dirs(app_name, target_apk_file_name):
    ...
    print(f"[DEBUG] class_files path: {class_files}")
    
    if not os.path.exists(class_files):
        print(f"[ERROR] File does not exist: {class_files}")
        raise FileNotFoundError(f"Cannot find class_files.json at {class_files}")

    # 打开 JSON 文件
    with open(class_files, "r", encoding="utf-8") as tmp_file:
        try:
    ...
    # 打印 JSON 中的键列表
    print(f"[DEBUG] Keys in class_files.json: {list(tmp_file_dict.keys())}")

    if target_apk_file_name not in tmp_file_dict:
        print(f"[ERROR] target_apk_file_name '{target_apk_file_name}' not found in class_files.json")
        raise KeyError(f"'{target_apk_file_name}' not found in class_files.json")

    class_source_files_dict = tmp_file_dict[target_apk_file_name]

    class_files_dirs = class_source_files_dict.get('classfiles', [])
    source_files_dirs = class_source_files_dict.get('sourcefiles', [])

    print(f"[DEBUG] classfiles: {class_files_dirs}")
    print(f"[DEBUG] sourcefiles: {source_files_dirs}")

    assert len(class_files_dirs) != 0 and len(source_files_dirs) != 0, "[ERROR] classfiles or sourcefiles is empty"
