...
def run_new_monkey(apk, avd_serial, avd_name, output_dir, testing_time, event_counts):
    command = 'run_monkey.ps1 %s %s %s %s' % (apk, avd_serial, avd_name, output_dir, testing_time, event_counts)
    print('execute monkey: %s' % command)
    os.system(command)
    ...
    while 0 <= apk_index < number_of_apks:
        ...
        for avd_serial in avd_serial_list:
            ...
            if args.monkey:
                ...
                #                                 login_script,))
                p.apply_async(run_new_monkey, args=(current_apk, avd_serial, args.avd_name,
...
if __name__ == '__main__':
    ...
    ap.add_argument('--time', type=str, default='60', help="the fuzzing time in minutes (e.g., 60 mins), default: 60")
    ap.add_argument('--repeat', type=int, default=3, help="the repeated number of runs, default: 1")
    ...
    ap.add_argument('--event_count', type=int, default=None, help="event counts, defaut=3600")
